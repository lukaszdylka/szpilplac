...
(function(){
  ...
  function sourceFor(link){
    var saved=link.getAttribute("data-support-source");
    if(saved)return saved.replace(/^szpilplac-?/,"")||"link";
  ...
  function urlFor(source){
    return BASE_URL+"?utm_source=szpilplac&utm_medium=website&utm_campaign=wsparcie&utm_content="+encodeURIComponent(source||"link");
  }
  function isHomePage(){
  ...
  function normalizeLinks(){
    document.querySelectorAll('a[href^="'+BASE_URL+'"]').forEach(function(link){
      var source=sourceFor(link);
      link.setAttribute("data-support-source",source);
      link.href=urlFor(source);
  ...
  function render(){
    ...
    if(!side)return;
    addStyle();
    var text=copy(),card=document.createElement("a");
    card.id="szpSupportCoffee";
    card.className="szp-support-card";
    card.href=urlFor("card");
    card.target="_blank";
    card.rel="noopener noreferrer";
    card.setAttribute("data-support-source","card");
    card.setAttribute("aria-label",text.button+" — otwiera buycoffee.to w nowej karcie");
    card.innerHTML=
      '<span class="szp-support-icon" aria-hidden="true"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M4 8h12v6a4 4 0 0 1-4 4H8a4 4 0 0 1-4-4V8z"></path><path d="M16 10h2.2a2.2 2.2 0 0 1 0 4.4H16"></path><path d="M7 4.5c0 .8-.7.9-.7 1.6M10 4.5c0 .8-.7.9-.7 1.6M13 4.5c0 .8-.7.9-.7 1.6"></path><path d="M4 20h13"></path></svg></span>'+
      '<span class="szp-support-copy"><span class="szp-support-kicker">'+text.kicker+'</span><span class="szp-support-title">'+text.title+'</span><span class="szp-support-text">'+text.text+'</span></span>'+
